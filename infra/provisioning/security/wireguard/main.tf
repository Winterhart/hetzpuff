...
resource "null_resource" "wireguard" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "while (ps aux | grep apt-get | grep -v grep > /dev/null); do : sleep 1; done",
  ...
  provisioner "remote-exec" {
    inline = [
      "echo '... wg -> joining and linking and reloading... '",
      "while (ps aux | grep apt-get | grep -v grep > /dev/null); do : sleep 1; done",
  ...
  provisioner "remote-exec" {
    inline = [
      "echo '(II).... wg -> reloading again service ....'",
      "systemctl enable overlay-route.service",
      "systemctl daemon-reload",
      "systemctl restart overlay-route.service"
    ]
  }
}
